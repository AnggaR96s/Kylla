...
async def _(event):
    ...
    packname = f"@UghteaSlayer's Sticker Pack"
    packshortname = f"UghteaSlayer_{userid}"  # format: Uni_Borg_userid
    ...
    if is_a_s:
        ...
        packname = f"@UghteaSlayer's @AnimatedStickerPack"
        if userid == 719877937:
            packshortname = "SnapDragon_Animated"
        else:
            packshortname = f"UghteaSlayer_{userid}_as" # format: Uni_Borg_userid
